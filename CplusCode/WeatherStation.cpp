...
bool WeatherStation::ReadRowFromWeatherStation(vector<ARDATA_c_t> &data_converted, char *buffer, int row){
    ...
    for(int i = 0; i < 5; i++){
        if(i >= row){
            ...
            if(size_data > 0){
                int date1 = (data_converted[data_converted.size()-1].year - 2000) + (data_converted[data_converted.size()-1].month * 32) +  data_converted[data_converted.size()-1].day +((data_converted[data_converted.size()-1].hour+1) *60) + data_converted[data_converted.size()-1].minutes;
                int date2 = (data_c.year -2000 )+ (data_c.month *32) + data_c.day + ((data_c.hour+1) * 60) + (data_c.minutes );
